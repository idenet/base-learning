...
function createStore(state, stateChange) {
  const linsteners = [] // 观察者列表
  const subscribe = linstener => linsteners.push(linstener) // 将观察者传入观察者列表
...
let oldState = store.getState() // 缓存老state
store.subscribe(() => {
...
renderApp(store.getState()) // 首次渲染到页面上
setTimeout(() => {
  store.dispatch({ type: 'UPDATE_TITLE_TEXT', text: '《React.js 小书》' })
  store.dispatch({ type: 'UPDATE_TITLE_COLOR', color: 'blue' }) // 修改标题颜色
  // 之后无论dispatch多少次都不用重新renderapp
}, 4000)
